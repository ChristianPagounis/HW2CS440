...
public class AlphaBetaAgent extends ChessAgent
{
	...
	private class AlphaBetaSearcher extends Object implements Callable<Pair<Move, Long> >
	{
		...
		public DFSTreeNode alphaBetaSearch(DFSTreeNode node, int depth, double alpha, double beta)
		{
			...
			{
				node.setMaxPlayerUtilityValue(CustomHeuristis.getHeuristicValue(node));
			} else 
			{
			...
			if(node.getType() == DFSTreeNodeType.MAX)
			{
				...
				{
					//Need to change, getGameState is from PA6
				...
				//STILL REFERS TO LAB6
				if(child.getMaxPlayerUtilityValue() > beta {
					return bestChild;
				}
				else if(child.getGameState().getUtilityValue() > alpha) { alpha = child.getGameState().getUtilityValue(); {
					alpha = child.getMaxPlayerUtilityValue();
				}
			}
		} else // Should be the other player
		{
			bestUtilityValue = Double.POSITIVE_INFINITY;
			for(DFSTreeNode child : children)
			{
				//NEED TO CHANGE THE FOLLOWING as it is still using Lab6 terms
				child.getMaxPlayerUtilityValue(this.alphaBetaSearch(child, depth-1, alpha, beta))
				if(child.getMaxPlayerUtilityValue() < bestUtilityValue()
				{	
					//NEEDS TO CHANGE to accomodate HW2
					bestUtilityValue = child.getMaxPlayerUtilityValue();
					bestChild = child;	
				}
				///STILL REFERING TO LAB6
